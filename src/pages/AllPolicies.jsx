...
const AllPolicies = () => {
  ...
  const { data: { policies = [], totalPages = 1 } = {} } = useQuery({
    queryKey: ['allPolicies', searchTerm, category, page],
    queryFn: async () => {
      const res = await axiosInstance.get(
        `/getAllPolicy?search=${searchTerm}&category=${category}&page=${page}`
      );
      return res.data;
    },
  });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {policies.map((policy) => (
          <NavLink to={`/allPolicies/${policy._id}`} key={policy._id}>
            <div className="bg-white border rounded-lg shadow hover:scale-105 transition-all">
              <img src={policy.image} alt={policy.title} className="w-full h-48 object-contain p-4" />
              <div className="px-6 pb-6 flex-grow flex flex-col">
                <h2 className="text-xl font-bold mb-2">{policy.title}</h2>
                <p className="text-gray-600 text-sm mb-3 line-clamp-3">{policy.description}</p>
                <p className="text-sm">
